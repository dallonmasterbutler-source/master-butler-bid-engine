...
- **Phone edition** — a single self-contained web page (Stitch-styled, embedded
  fonts, light/dark). No accounts, no server: each person opens the link on
  their phone and their data lives only in their own browser (“Add to Home
  Screen” and it runs like an app). It never scrapes amway.com; the product
  cards are baked in. It ships as **two builds from one source**:
  - **`personal_edition.html`** — the FULL app (the source of truth), including
    the **Plan** tab (personal errands/food-list → stores → Google Calendar).
    For you + your wife.
  - **`team_edition.html`** — the SHARE build: everything **except** the Plan
    tab, generated by stripping the `PLAN:START…PLAN:END`-marked blocks. Run
    `python3 build_editions.py` to inject `products.json` into the source and
    regenerate the share build. (Distributing any build to others is still BSM
    under Amway’s QAS — clear it with your upline first.)
